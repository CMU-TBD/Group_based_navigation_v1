...
def get_noise(shape, noise_type):
    if noise_type == 'gaussian':
        return torch.randn(*shape).cuda('cuda:0')
    elif noise_type == 'uniform':
        return torch.rand(*shape).sub_(0.5).mul_(2.0).cuda('cuda:0')
...
class Encoder(nn.Module):
    ...
    def init_hidden(self, batch):
        return (
            torch.zeros(self.num_layers, batch, self.h_dim).cuda('cuda:0'),
            torch.zeros(self.num_layers, batch, self.h_dim).cuda('cuda:0')
        )
...
class PoolHiddenNet(nn.Module):
    ...
    def __init__(
        ...
        activation='leakyrelu', batch_norm=True, dropout=0.0
    ):
...
class SocialPooling(nn.Module):
    ...
    def __init__(
        self, h_dim=64, activation='leakyrelu', batch_norm=True, dropout=0.0,
